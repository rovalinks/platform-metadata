#!/bin/bash
# ==============================================================================
# Script to view labels/metadata for all governed resources (Labels & Tags map)
# ==============================================================================
...
echo "Listing labels and metadata for all test resources in $PROJECT..."
printf "%-35s | %-25s | \n" "RESOURCE TYPE" "RESOURCE NAME"
echo "------------------------------------------------------------------------"

# Project
printf "%-35s | %-25s | " "crm-project" "$PROJECT"
gcloud projects describe $PROJECT --format="value(labels)" 2>/dev/null || echo "N/A"

# AI Platform / Vertex AI
printf "%-35s | %-25s | " "aiplatform-dataset" "$PREFIX-vertex-ds"
gcloud ai datasets list --region=$REGION --project=$PROJECT --filter="displayName=$PREFIX-vertex-ds" --format="value(labels)" 2>/dev/null || echo "N/A"

# AlloyDB
printf "%-35s | %-25s | " "alloydb-cluster" "$PREFIX-alloy-cluster"
...
# Compute Engine Tags-only resources
printf "%-35s | %-25s | " "compute-network (Tags)" "$PREFIX-network"
gcloud compute networks describe $PREFIX-network --project=$PROJECT --format="value(description)" 2>/dev/null || echo "N/A"

printf "%-35s | %-25s | " "compute-firewall (Tags)" "$PREFIX-fw"
gcloud compute firewalls describe $PREFIX-fw --project=$PROJECT --format="value(description)" 2>/dev/null || echo "N/A"

# Container / GKE
printf "%-35s | %-25s | " "gke-cluster" "$PREFIX-gke"
...
# Redis
printf "%-35s | %-25s | " "redis-instance" "$PREFIX-redis"
...
# SQL & Storage
printf "%-35s | %-25s | " "cloud-sql" "$PREFIX-sql"
